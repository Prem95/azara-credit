...
scenario = st.sidebar.selectbox("Navigation", ("Credit Calculation", "Scenario 1", "Scenario 2"))
...
if scenario == "Scenario 1":
    ...
    AZARA_MARGIN = col1.slider("Azara Margin (%)", min_value=0.0, max_value=100.0, value=20.0, step=1.0)
    AWS_COST = col2.slider("AWS Cost ($/hour)", min_value=0.0, max_value=100.0, value=0.1, step=0.1)

    # Inputs for GPT Token
    col1, col2, col3 = st.columns((3, 1, 2))
    with col1:
        st.header("LLM")
        model = st.radio("Which model will you use?", ("gpt-4-8k", "gpt-4-32k", "gpt-3.5-turbo"))
    ...
    with col3:
        st.header("Document")
        uploaded_file = st.file_uploader("Upload a txt file", type=["pdf", "docx", "txt"])
        if uploaded_file:
            doc = None
